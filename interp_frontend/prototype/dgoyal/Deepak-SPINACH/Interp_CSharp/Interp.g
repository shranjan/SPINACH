...
@init {
	...
	'{' (el1=structvardec { retval.ret.setVarType($el1.ret);})+
	'}')END_OF_STATEMENT; 

structvardec returns [StructVariableDeclaration ret]
@init {
	retval.ret = new StructVariableDeclaration();
	}	
	:(TYPE { retval.ret.setType($TYPE.text);} 
	variable { retval.ret.setVar($variable.ret);}';');
